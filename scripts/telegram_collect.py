#!/usr/bin/env python3
"""Collect URLs posted in Telegram chats the bot can see and store readable content under telegram_inbox/."""
...
def render_markdown(entries: list[dict], day: str) -> str:
    ...
    for e in groups["ok"]:
        meta = " ｜ ".join(filter(None, [f"來源聊天：{e['chat']}", f"貼文時間：{e['posted_at'][:16]}", e.get("site") and f"站台：{e['site']}", e.get("published") and f"發布：{e['published']}"]))
    ...
    for e in groups["needs_manual_review"]:
        ...
        lines += [f"- {e['final_url']}（來源聊天：{e['chat']}）{hint}"]
    lines.append("")
...
def main() -> None:
    ...
    print(f"{len(updates)} updates processed, {len(new_entries)} new URLs")
